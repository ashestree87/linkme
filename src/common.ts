 ...
 */
export async function verifyLinkedInAuth(): Promise<{isValid: boolean, message: string}> {
  ...
  try {
    ...
    try {
      ...
      const isLoggedIn = await page.evaluate(() => {
        return !!document.querySelector('a[data-control-name="identity_profile_photo"]') ||
               !!document.querySelector('.global-nav__me-photo') ||
               !!document.querySelector('.feed-identity-module__member-photo');
      });
      ...
      if (isLoggedIn) {
        ...
        return {
          isValid: false,
          message: "LinkedIn authentication failed. Please update your LI_AT and CSRF cookies."
        };
      }
    } finally {
      await browser.close();
